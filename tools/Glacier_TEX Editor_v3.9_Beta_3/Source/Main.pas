...
type
  ...
  TfrmTEXeditor = class(TForm)
    ...
    procedure ZipMasterMessage(Sender: TObject; ErrCode: Integer;
      Message: String);
    procedure ZipMasterTotalProgress(Sender: TObject; TotalSize: Int64;
      PerCent: Integer);
    ...
    procedure chkI8onlyClick(Sender: TObject);
    procedure chkU8V8onlyClick(Sender: TObject);
  private
    texFile: TFileStream;
    iniFile: TIniFile;
    sortColumn: Integer;
    ...
    procedure exportI8(offset: integer; outname: string);
    procedure exportU8V8(offset: integer; outname, outname2: string);
    procedure exportDXT(dxtchar: char; offset: integer; outname: string);
    procedure exportBinCopy(offset: integer; outname: string);
...
// offset := 16;
  ...
  else if (lstTEX.SelCount = 1) and (lstTEX.Selected.SubItems.Strings[1] = 'I8  ') then
  begin

   importDialog.Title := 'Select TGA to import...';
   importDialog.Filter := 'Targa 8bpp Greyscale (*.TGA)|*.TGA';
...
// offset := 16;
    ...
    try
      setStatusMessage('TGA->I8: [1/5] Reading TGA file...');
      img8.LoadFromTGA8(importfile);

      setStatusMessage('TGA->I8: [2/5] Reading TEX file...');
      texFile.Seek(offset,soFromBeginning);
      texFile.Read(HDR,SizeOf(HMC_Tex_Entry));
      Get0(texFile);
      texFile.Read(fsize,4);
      ...
      for x := 0 to 255 do
      begin
        testgreyscale := testgreyscale and (img8.Palette[x].R = x) and (img8.Palette[x].G = x) and (img8.Palette[x].B = x);
      end;

      if not(testgreyscale) then
      begin
        raise Exception.Create('TGA is not greyscale!');
      end;

      setStatusMessage('TGA->I8: [3/5] Converting data to I8...');
      GetMem(Buffer,W*H);
      try
        for y := 0 to H-1 do
          for x := 0 to W-1 do
        ...
        if chkXbox.Checked then
        begin
          setStatusMessage('TGA->I8: [4/5] Writing Xbox I8 data to TEX file ('+inttostr(H*W)+' bytes)...');
          xbox := TXboxTexture.create;
          try
            xbox.intex.Write(Buffer^,H*W);
            xbox.reswizzle(W,H,1,1);
        ...
        begin
          setStatusMessage('TGA->I8: [4/5] Writing I8 data to TEX file ('+inttostr(H*W)+' bytes)...');
          texFile.Write(Buffer^,H*W);
          //texFile.Seek(W*H,soFromCurrent);
        end;
        ...
        begin
          ...
          if bmp[x-1].Height > 1 then
            ...
            bmp[x].height := 1;
          setStatusMessage('TGA->I8: [4/5] Computing mipmap '+inttostr(x+1)+' ('+inttostr(bmp[x].Width)+'x'+inttostr(bmp[x].height)+')...');
          Strecth(bmp[x-1],bmp[x],ResampleFilters[5].Filter,ResampleFilters[5].Width);
          mipmap[x-1] := TSaveImage32.Create;
          mipmap[x-1].LoadFromTBitmap(bmp[x]);
          H := mipmap[x-1].Height;
          ...
          for my := 0 to H-1 do
            for mx := 0 to W-1 do
            begin
              Buffer[(my*W)+mx] := mipmap[x-1].Pixels[mx][my].R;
            end;
          texFile.Read(fsize,4);
          //bmp[x].SaveToFile('c:\BD\'+inttostr(x)+' '+inttostr(fsize)+'.bmp');
          if chkXbox.Checked then
          begin
            setStatusMessage('TGA->I8: [4/5] Writing Xbox mipmap '+inttostr(x+1)+' ('+inttostr(H*W*4)+' bytes)...');
            xbox := TXboxTexture.create;
            try
              xbox.intex.Write(Buffer^,H*W);
              xbox.reswizzle(W,H,1,1);
          ...
          begin
            setStatusMessage('TGA->I8: [4/5] Writing mipmap '+inttostr(x+1)+' ('+inttostr(H*W*4)+' bytes)...');
            // Comment out for actual import
            texFile.Write(Buffer^,H*W);
            //texFile.Seek(W*H,soFromCurrent);
           end;
        ...
        setStatusMessage('TGA->I8: [5/5] Done! (Import successfull)');
      finally
        FreeMem(Buffer);
      end;
    except
...
var HDR: TEX_Header;
    ENT: TEX_Entry;
    NumE, NumShown, NumRGBA, NumDXT1, NumDXT3, NumPALN, NumI8, NumU8V8: cardinal;
    x, y, per, OldPer: integer;
    nam: string;
...
begin
  ...
  ProgressBar2.Caption := 'Parsing TEX... %d%%';
  ...
  begin
    ...
    try
      ...
      if (((HDR.ID3 <> 3) and (HDR.ID3 <> 0)) or (HDR.ID4 <> 4) or (HDR.IndexOffset >= texFile.Size) or (HDR.UnknownOffset >= texFile.Size) or ((HDR.UnknownOffset-HDR.IndexOffset) <> $2000)) then
      begin
        closeTEX;
      ...
      begin
        ...
        texFile.Seek(HDR.IndexOffset,soFromBeginning);
        ...
        begin
          ...
          if per >= OldPer + 20 then
          begin
            ProgressBar2.Percent := per;
            oldPer := per;
          end;
          ...
          if ENT.Type1 = 'ABGR' then
            ...
            inc(NumPALN)
          else if ENT.Type1 = '8V8U' then
            inc(NumU8V8)
          else if ENT.Type1 = '  8I' then
            inc(NumI8)
          else
          begin
//            inc(NumUnk);
          ...
          or (chkU8V8only.Checked and (ENT.Type1 = '8V8U'))
          or (chkI8only.Checked and (ENT.Type1 = '  8I'))
          or alwaysShow
          then
          begin
            inc(NumShown);
            ...
            item.SubItems.Add(inttostr(ENT.Index));
        ...
        ProgressBar2.Percent := 100;
        SetStatusMessage('Parse TEX: Done! '+inttostr(NumRGBA)+' RGBA / '+inttostr(NumDXT1)+' DXT1 / '+inttostr(NumDXT3)+' DXT3 / '+inttostr(NumPALN) +' PALN / '+inttostr(NumU8V8) +' U8V8 / '+inttostr(NumI8) +' I8 - ['+inttostr(NumE-NumShown)+' hidden]');
...
begin
  ...
  begin
    ...
    else if (Item.SubItems[1] = 'I8  ') then
    begin
      butImport.Enabled := true;
      butExport.Enabled := true;
    end
    else if (Item.SubItems[1] = 'U8V8') then
    begin
      butImport.Enabled := false;
      butExport.Enabled := true;
    end
...
begin
  ...
  begin
    ...
    IniFile.WriteBool('HMCEditor','ShowOnlyU8V8',false);
    IniFile.WriteBool('HMCEditor','ShowOnlyI8',false);
  ...
  else if not(IniFile.ValueExists('HMCEditor','ShowOnlyU8V8')) then
  begin
    IniFile.WriteBool('HMCEditor','ShowOnlyU8V8',false);
    IniFile.UpdateFile;
  end
  else if not(IniFile.ValueExists('HMCEditor','ShowOnlyI8')) then
  begin
    IniFile.WriteBool('HMCEditor','ShowOnlyI8',false);
    IniFile.UpdateFile;
  end
  ...
  chkU8V8only.Checked := IniFile.ReadBool('HMCEditor','ShowOnlyU8V8',false);
  chkI8only.Checked := IniFile.ReadBool('HMCEditor','ShowOnlyI8',false);
  ...
  if not(DirectoryExists(IniFile.ReadString('HMCEditor','TempPath',ExtractFilePath(Application.ExeName)+'Temp\'))) then
  ...
  if not(IniFile.ReadBool('HMCEditor','ReadWarning',false)) then
    if (MessageDlg('Welcome to'+#10+'Glacier TEX Editor'+#10+#10+'This tool can modify .TEX files (inside .ZIP archives) from Glacier engine based games.'+#10+'That means (at least):'+#10+'- Freedom Fighters'+#10+'- Hitman: Silent Assassin'+#10+'- Hitman: Contracts'+#10+'You should consider making a backup of those files.'+#10+#10+'This tool is provided with no guarantee.'+#10+'It is in no way an official tool.'+#10+#10+'Do you understand and accept that this tool might mess up your TEX files ?',mtConfirmation,[mbYes, mbNo],0) = mrNo) then
    begin
  ...
  SetStatusMessage('Information: ZipMaster v'+ZIPMASTERBUILD+' loaded! (DelZip DLL v'+ZipMaster.Dll_Version+')');

end;

procedure TfrmTEXeditor.butExportClick(Sender: TObject);
var outsrc, outsrc2, outdir, origname: string;
...
begin
  ...
  begin
    ...
    outsrc := StringReplace(extractfilename(txtSource.Text),'.','_',[rfReplaceAll]) + '_' + copy('0000'+lstTEX.Selected.SubItems.Strings[3],length(lstTEX.Selected.SubItems.Strings[3]),5)+ '_'+outsrc + '.dds';
  ...
  begin
    ...
    outsrc := StringReplace(origname,'\','_',[rfReplaceAll]);
    outsrc := StringReplace(outsrc,'/','_',[rfReplaceAll]);
    outsrc := StringReplace(extractfilename(txtSource.Text),'.','_',[rfReplaceAll]) + '_' + copy('0000'+lstTEX.Selected.SubItems.Strings[3],length(lstTEX.Selected.SubItems.Strings[3]),5)+ '_'+outsrc + '.tga';
    saveDialog.FileName := outsrc;
    saveDialog.Filter := 'Targa 32bpp with alpha channel (*.TGA)|*.TGA';
    saveDialog.Title := 'Save TGA file...';
  ...
  begin
    ...
    setStatusMessage('I8->TGA[0/4] Asking for output filename...');
    offset := strtoint(lstTEX.Selected.Caption);
    origname := lstTEX.Selected.SubItems.Strings[0];
    ...
    outsrc := StringReplace(extractfilename(txtSource.Text),'.','_',[rfReplaceAll]) + '_' + copy('0000'+lstTEX.Selected.SubItems.Strings[3],length(lstTEX.Selected.SubItems.Strings[3]),5)+ '_'+outsrc + '.tga';
    saveDialog.FileName := outsrc;
    saveDialog.Filter := 'Truevision Targa 8Bpp Greyscale (*.TGA)|*.TGA';
    saveDialog.Title := 'Save TGA file...';

    if saveDialog.Execute then
      try
        IniFile.WriteString('HMCEditor','ExportPath',ExtractFilePath(saveDialog.FileName));
        exportI8(offset,saveDialog.FileName)
      except
        on e:exception do
        begin
          SetStatusMessage('Error: '+E.Message);
      ...
      setStatusMessage('I8->TGA: Canceled.');

  end
  else if (lstTEX.SelCount = 1) and (lstTEX.Selected.SubItems.Strings[1] = 'U8V8') then
  begin

    setStatusMessage('U8V8->U8V8[0/4] Asking for output filename...');
    offset := strtoint(lstTEX.Selected.Caption);
    origname := lstTEX.Selected.SubItems.Strings[0];
    ...
    outsrc := StringReplace(origname,'\','_',[rfReplaceAll]);
    outsrc := StringReplace(outsrc,'/','_',[rfReplaceAll]);
    outsrc := StringReplace(extractfilename(txtSource.Text),'.','_',[rfReplaceAll]) + '_' + copy('0000'+lstTEX.Selected.SubItems.Strings[3],length(lstTEX.Selected.SubItems.Strings[3]),5)+ '_'+outsrc;
    outsrc2 := outsrc + '_V8.tga';
    outsrc := outsrc + '_U8.tga';
    saveDialog.FileName := outsrc;
    saveDialog.Filter := 'Truevision Targa 8Bpp Greyscale (*.TGA)|*.TGA';
    saveDialog.Title := 'Save TGA U8 file...';

    if saveDialog.Execute then
    begin
      outsrc := saveDialog.FileName;
      saveDialog.FileName := outsrc2;
      saveDialog.Filter := 'Truevision Targa 8Bpp Greyscale (*.TGA)|*.TGA';
      saveDialog.Title := 'Save TGA V8 file...';
      if saveDialog.Execute then
        try
          IniFile.WriteString('HMCEditor','ExportPath',ExtractFilePath(saveDialog.FileName));
          exportU8V8(offset,outsrc,saveDialog.FileName)
        except
          on e:exception do
          begin
            SetStatusMessage('Error: '+E.Message);
        ...
        setStatusMessage('I8->U8V8: Canceled.');
    end
    else
      setStatusMessage('I8->U8V8: Canceled.');
    ...
    begin
      ...
      for x := 0 to lstTEX.Items.Count-1 do
        ...
        begin
          ...
          outsrc := StringReplace(origname,'\','_',[rfReplaceAll]);
          outsrc := StringReplace(outsrc,'/','_',[rfReplaceAll]);
          outsrc := StringReplace(extractfilename(txtSource.Text),'.','_',[rfReplaceAll]) + '_' + copy('0000'+lstTEX.Items.Item[x].SubItems.Strings[3],length(lstTEX.Items.Item[x].SubItems.Strings[3]),5)+ '_'+outsrc;
          ...
          else if lstTEX.Items.Item[x].SubItems.Strings[1] = 'I8  ' then
          begin
            exportI8(offset,outsrc + '.tga');
          end;
...
begin
  ...
  case sortColumn of
    ...
    4: Compare := strtoint(Item1.SubItems.Strings[3]) - strtoint(Item2.SubItems.Strings[3]);
    5: Compare := CompareText(Item1.SubItems.Strings[4],Item2.SubItems.Strings[4]);
  end;
...
//  or ((currentZIPaction = tcaUpdate) and (ErrCode > 0) and (ErrCode <= 11000)) then
  if ((ErrCode > 0) and (ErrCode <> 10204)) then
  begin
...
begin
  ...
  result := false;
  trunc := uppercase(trunc)+'.TEX';

  for x := 0 to Zipmaster.ZipContents.Count-1 do
  begin
    result := (pos(trunc, uppercase(Zipmaster.DirEntry[x]^.FileName)) > 0);
    if result then
      break;
  end;
...
procedure TfrmTEXeditor.ZipMasterProgress(Sender: TObject;
  ProgrType: ProgressType; Filename: String; FileSize: Int64);
begin
...
procedure TfrmTEXeditor.exportBinCopy(offset: integer; outname: string);
var HDR: HMC_TEX_Entry;
    outFile: TFileStream;
    fsize: cardinal;
begin
...
procedure TfrmTEXeditor.exportI8(offset: integer; outname: string);
var x, y, W, H, fsize: integer;
    img8: TSaveImage;
    HDR: HMC_TEX_Entry;
    Buffer: PByteArray;
...
begin
  ...
  setStatusMessage('I8->TGA: [1/4] Reading I8 header & data in TEX file...');
  texFile.Seek(offset,soFromBeginning);
  texFile.Read(HDR,SizeOf(HMC_Tex_Entry));
  Get0(texFile);
  texFile.Read(fsize,4);
...
//    raise Exception.Create('Not an PALN texture!');
  ...
  try
    if chkXbox.Checked then
    begin
      setStatusMessage('I8->TGA: [1/4] Reading I8 header & data in TEX file... Xbox unswizzle...');
      xbox := TXboxTexture.create;
      try
        xbox.intex.CopyFrom(texFile,fsize);
        xbox.unswizzle(HDR.Width,HDR.Height,1,1);
    ...
    setStatusMessage('I8->TGA: [2/4] Computing I8 data to TGA format...');
    img8.SetSizePal(HDR.Width, HDR.Height,256,false);
    ...
    setStatusMessage('I8->TGA: [3/4] Computing greyscale palette...');
    ...
    setStatusMessage('I8->TGA: [4/4] Saving TGA 8bpp (Greyscale)...');
    img8.SaveToTGA8(outname);
    setStatusMessage('I8->TGA: Done! (Export successfull) [Output is TGA 8Bpp Greyscale]');
  except
    on E: Exception do
    begin
      SetStatusMessage('Error: '+E.Message);
...
procedure TfrmTEXeditor.exportU8V8(offset: integer; outname, outname2: string);
var x, y, W, H, fsize: integer;
    img8u, img8v: TSaveImage;
    HDR: HMC_TEX_Entry;
    Buffer: PByteArray;
    xbox: TXBoxTexture;
begin

  setStatusMessage('U8V8->2TGA: [1/4] Reading U8V8 header & data in TEX file...');
  texFile.Seek(offset,soFromBeginning);
  texFile.Read(HDR,SizeOf(HMC_Tex_Entry));
  Get0(texFile);
  texFile.Read(fsize,4);
...
//    raise Exception.Create('Not an PALN texture!');
  ...
  img8u := TSaveImage.Create;
  img8v := TSaveImage.Create;
  GetMem(Buffer,fsize);
  try
    if chkXbox.Checked then
    begin
      setStatusMessage('U8V8->2TGA: [1/4] Reading U8V8 header & data in TEX file... Xbox unswizzle...');
      xbox := TXboxTexture.create;
      try
        xbox.intex.CopyFrom(texFile,fsize);
        xbox.unswizzle(HDR.Width,HDR.Height,2,1);
        xbox.outtex.Seek(0,soFromBeginning);
        xbox.outtex.Read(Buffer^,fsize);
      finally
        xbox.destroy;
    ...
    setStatusMessage('U8V8->2TGA: [2/4] Computing U8V8 data to TGA format...');
    img8u.SetSizePal(HDR.Width, HDR.Height,256,false);
    img8v.SetSizePal(HDR.Width, HDR.Height,256,false);
    ...
    for y := 0 to H-1 do
      for x := 0 to W-1 do
      begin
        img8u.Pixels[x][y] := Buffer[(y*W*2)+x*2];
        img8v.Pixels[x][y] := Buffer[(y*W*2)+x*2+1];
      end;

    setStatusMessage('U8V8->2TGA: [3/4] Computing greyscale palette...');
    ...
    setStatusMessage('U8V8->2TGA: [4/4] Saving 2x TGA 8bpp (Greyscale)...');
    img8u.SaveToTGA8(outname);
    img8v.SaveToTGA8(outname2);
    setStatusMessage('U8V8->2TGA: Done! (Export successfull) [Output is 2x TGA 8Bpp Greyscale]');
  except
    on E: Exception do
    begin
      SetStatusMessage('Error: '+E.Message);
...
procedure TfrmTEXeditor.chkI8onlyClick(Sender: TObject);
begin

  IniFile.WriteBool('HMCEditor','ShowOnlyI8',chkI8only.Checked);

end;

procedure TfrmTEXeditor.chkU8V8onlyClick(Sender: TObject);
begin

  IniFile.WriteBool('HMCEditor','ShowOnlyU8V8',chkU8V8only.Checked);
